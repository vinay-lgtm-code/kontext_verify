// ============================================================================
// Deploy KontextAnchor to Base Sepolia
...
async function main() {
  // Read from env var or .env.deployer file
  let privateKey = process.env['PRIVATE_KEY'];
  if (!privateKey) {
    try {
      const { readFileSync } = await import('fs');
      const content = readFileSync('contracts/.env.deployer', 'utf8');
      privateKey = content.match(/PRIVATE_KEY=(0x[a-fA-F0-9]+)/)?.[1];
    } catch {
      // file doesn't exist
    }
  }
  if (!privateKey) {
    console.error('No private key found. Run: npx tsx contracts/setup-deployer.ts');
    process.exit(1);
  }
