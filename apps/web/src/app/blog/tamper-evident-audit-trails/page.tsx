  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="prose-kontext">
        ...
        <CodeBlock
          code={`import { Kontext, DigestChain, verifyExportedChain } from 'kontext-sdk';
...
await ctx.logTransaction({
  ...
  from: '0xAgent...abc',
  to: '0xVendor...def',
  agent: 'treasury-agent-v3',
});

// Export and verify
const audit = await ctx.exportAudit({ format: 'json' });
const verification = verifyExportedChain(audit.chain);
...
if (result.valid) {
  ...
  console.error('Found digest:', result.foundDigest);
        ...
        <CodeBlock
          code={`const ctx = new Kontext({
  chain: 'base',
  anchoring: {
    enabled: true,
    interval: '1h',          // anchor the latest digest every hour
    contract: '0xKontext...anchor',
  },
});

// Business as usual -- anchoring happens automatically
await ctx.logTransaction({
  action: 'usdc_transfer',
  amount: '12500.00',
  currency: 'USDC',
  from: '0xAgent...abc',
  to: '0xSupplier...789',
  agent: 'procurement-agent-v1',
});

// Verify including on-chain anchors
const verification = await ctx.verifyWithAnchors();
console.log(verification.anchorsChecked); // 24
console.log(verification.allAnchorsValid); // true`}
          language="typescript"
